...
5. 运行 `vp run @app/template-api#push`（开发环境）

### 修改现有表

1. 修改 schema 文件中的表定义
2. 运行 `vp run @app/template-api#push`（开发环境）
3. 如果需要迁移数据，编写迁移脚本

### 生产部署

```bash
vp run @app/template-api#generate  # 生成迁移文件
vp run @app/template-api#migrate   # 执行迁移
```
