...
const now = Date.now()
const suffix =
  String.fromCharCode(97 + (now % 26)) + // 1st letter / 第一个字母
...
async function cleanupTestRoles(): Promise<void> {
  try {
    await db.transaction(async (tx) => {
      ...
      const testRoles = await tx.select({ id: systemRoles.id }).from(systemRoles).where(like(systemRoles.id, 'test_%'))
      ...
      await tx.delete(systemUserRoles).where(or(...roleIds.map((id) => eq(systemUserRoles.roleId, id))))

      // Delete Casbin permission rules (ptype = "p") / 删除 Casbin 权限规则 (ptype = "p")
      await tx.delete(casbinRule).where(and(eq(casbinRule.ptype, 'p'), or(...roleIds.map((id) => eq(casbinRule.v0, id)))))

      // Delete Casbin role inheritance rules (ptype = "g") / 删除 Casbin 角色继承规则 (ptype = "g")
      await tx.delete(casbinRule).where(
        and(
      ...
      await tx.delete(systemRoles).where(like(systemRoles.id, 'test_%'))
    })
  } catch (error) {
...
describe('system role routes', () => {
  ...
  describe('authentication & authorization', () => {
    ...
    it('should deny access with user token (no admin permissions)', async () => {
      const response = await client.system.roles.$get({ query: {} }, { headers: getAuthHeaders(userToken) })
  ...
  describe('get /system/role - list roles', () => {
    ...
    it('should support filtering by id', async () => {
      const response = await client.system.roles.$get(
        {
          query: {
            filters: JSON.stringify([{ field: 'id', operator: 'contains', value: 'admin' }]),
          },
        },
        { headers: getAuthHeaders(adminToken) },
      )
      ...
      if (response.status === HttpStatusCodes.OK) {
        ...
        if (items.length > 0) {
          expect(items.some((role: { id: string }) => role.id.includes('admin'))).toBe(true)
        }
      }
    })
    ...
    it('should support filtering by name', async () => {
      const response = await client.system.roles.$get(
        {
          query: {
            filters: JSON.stringify([{ field: 'name', operator: 'contains', value: '管理' }]),
          },
        },
        { headers: getAuthHeaders(adminToken) },
      )
      ...
      if (response.status === HttpStatusCodes.OK) {
        ...
        const items = Array.isArray(json.data) ? json.data : []
        if (items.length > 0) {
          expect(items.some((role: { name: string }) => role.name.includes('管理'))).toBe(true)
        }
      }
    })
    ...
    it('should list roles with parent roles information', async () => {
      ...
      const response = await client.system.roles.$get(
        {
          query: {
            filters: JSON.stringify([{ field: 'id', operator: 'eq', value: childRoleId }]),
          },
        },
        { headers: getAuthHeaders(adminToken) },
      )
  ...
  describe('get /system/role/{id} - get role', () => {
    ...
    it('should return 404 for non-existent role', async () => {
      ...
      const response = await client.system.roles[':id'].$get({ param: { id: nonExistentId } }, { headers: getAuthHeaders(adminToken) })
    ...
    it('should get role details', async () => {
      const response = await client.system.roles[':id'].$get({ param: { id: roleId } }, { headers: getAuthHeaders(adminToken) })
    ...
    it('should get role details with parent roles', async () => {
      ...
      const response = await client.system.roles[':id'].$get({ param: { id: childRoleId } }, { headers: getAuthHeaders(adminToken) })
  ...
  describe('patch /system/role/{id} - update role', () => {
    ...
    beforeAll(async () => {
      ...
      const adminResponse = await client.system.roles.$get(
        {
          query: {
            filters: JSON.stringify([{ field: 'id', operator: 'eq', value: 'admin' }]),
          },
        },
        { headers: getAuthHeaders(adminToken) },
      )
  ...
  describe('delete /system/role/{id} - delete role', () => {
    ...
    it('should return 404 for non-existent role', async () => {
      const nonExistentId = 'non_existent_role'
      const response = await client.system.roles[':id'].$delete({ param: { id: nonExistentId } }, { headers: getAuthHeaders(adminToken) })
  ...
  describe('get /system/role/{id}/permissions - get role permissions', () => {
    ...
    it('should get role permissions', async () => {
      const response = await client.system.roles[':id'].permissions.$get({ param: { id: roleId } }, { headers: getAuthHeaders(adminToken) })
    ...
    it('should get admin role permissions', async () => {
      ...
      if (adminResponse.status === HttpStatusCodes.OK) {
        ...
        if (items.length > 0) {
          ...
          const response = await client.system.roles[':id'].permissions.$get({ param: { id: adminRoleId } }, { headers: getAuthHeaders(adminToken) })
          ...
          if (response.status === HttpStatusCodes.OK) {
            ...
            expect(permJson.data).toHaveProperty('permissions')
            expect(permJson.data).toHaveProperty('groupings')
            expect(Array.isArray(permJson.data.permissions)).toBe(true)
            ...
            if (permJson.data.permissions.length > 0) {
              const firstPerm = permJson.data.permissions[0]
  ...
  describe('put /system/role/{id}/permissions - save role permissions', () => {
    ...
    it('should update permissions (replace existing)', async () => {
      ...
      const verifyResponse = await client.system.roles[':id'].permissions.$get({ param: { id: roleId } }, { headers: getAuthHeaders(adminToken) })
    ...
    it('should clear all permissions when empty array provided', async () => {
      ...
      if (verifyResponse.status === HttpStatusCodes.OK) {
        ...
        expect(verifyJson.data.permissions.length).toBe(0)
      }
    })
    ...
    it('should allow updating permissions without inherited permissions', async () => {
      ...
      const getPermsResponse = await client.system.roles[':id'].permissions.$get({ param: { id: childRoleId } }, { headers: getAuthHeaders(adminToken) })
      ...
      const verifyResponse = await client.system.roles[':id'].permissions.$get({ param: { id: childRoleId } }, { headers: getAuthHeaders(adminToken) })
    ...
    it('should prevent adding duplicate inherited permissions', async () => {
      ...
      if (response.status === HttpStatusCodes.BAD_REQUEST) {
        ...
        expect(json.message).toContain('不能重复添加已继承的权限')
        expect(json.message).toContain('/system/users:GET')
      }
    })
  })
