...
export type RefineLogger = {
  warn: (data: Record<string, unknown>, message: string) => void
  error: (data: Record<string, unknown>, message: string) => void
}

const noopLogger: RefineLogger = {
  warn: () => {},
  error: () => {},
}
...
export class FiltersConverter {
  ...
  constructor(table: PgTable, logger: RefineLogger = noopLogger) {
    this.table = table
    this.logger = logger
  }
  ...
  private convertLogicalFilter(filter: LogicalFilter): SQL<unknown> | undefined {
    ...
    if (!column) {
      this.logger.warn({ field: filter.field }, '[查询过滤]: 未知字段')
      return undefined
    }
    ...
    try {
      switch (operator) {
        ...
        default:
          this.logger.warn({ operator }, '[查询过滤]: 不支持的操作符')
          return undefined
      }
    } catch (error) {
      this.logger.error({ field: filter.field, error: error instanceof Error ? error.message : String(error) }, '[查询过滤]: 转换过滤器错误')
      return undefined
    }
  }
  ...
  private convertConditionalFilter(filter: ConditionalFilter): SQL<unknown> | undefined {
    ...
    const conditions = value.map((subFilter) => this.convertFilter(subFilter)).filter(Boolean) as SQL<unknown>[]
    ...
    switch (operator) {
      ...
      default:
        this.logger.warn({ operator }, '[查询过滤]: 不支持的条件操作符')
        return undefined
    }
  }
...
export function convertFiltersToSQL(filters: CrudFilters | undefined, table: PgTable, logger?: RefineLogger): SQL<unknown> | undefined {
  if (!filters) return undefined
  return new FiltersConverter(table, logger).convert(filters)
}
...
export class SortersConverter {
  ...
  private convertSorter(sorter: CrudSorting[number]): SQL<unknown> | undefined {
    ...
    if (!column) {
      this.logger.warn({ field: sorter.field }, '[查询排序]: 未知排序字段')
      return undefined
    }
    ...
    try {
      switch (sorter.order) {
        ...
        case 'desc':
          return desc(column)
        default:
          this.logger.warn({ order: sorter.order }, '[查询排序]: 无效的排序方向')
          return undefined
      }
    } catch (error) {
      this.logger.error({ field: sorter.field, error: error instanceof Error ? error.message : String(error) }, '[查询排序]: 转换排序条件错误')
      return undefined
    }
  }
...
export function convertSortersToSQL(sorters: CrudSorting | undefined, table: PgTable, logger?: RefineLogger): SQL<unknown>[] {
  if (!sorters) return []
  return new SortersConverter(table, logger).convert(sorters)
}

/** Validate sorting fields / 验证排序字段 */
export const validateSorterFields: SorterFieldsValidator = (sorters, table, allowedFields?) => {
  const validColumns = allowedFields ? [...allowedFields] : Object.keys(table)
  const invalidFields = sorters.map((sorter) => sorter.field).filter((field) => !validColumns.includes(field))

  return { valid: invalidFields.length === 0, invalidFields: [...new Set(invalidFields)] }
}

/** Add default sorting / 添加默认排序 */
export function addDefaultSorting(sorters: CrudSorting | undefined, defaultField: string = 'createdAt', defaultOrder: 'asc' | 'desc' = 'desc'): CrudSorting {
