...
import type { ArrowFunctionExpression, CallExpression, MethodDefinition, Function as OxcFunction, VariableDeclaration } from '@oxc-project/types'
 ...
 * 生成唯一变量名
 */
function generateVariableName(code: string, index: number, prefix: string): string {
 ...
 */
function isZodCall(node: CallExpression, zodIdentifier: string): boolean {
  ...
  const isIdentifier = (n: AstNode | undefined) => n && (n.type === 'Identifier' || n.type === 'IdentifierReference' || n.type === 'IdentifierName')
  const isMemberExpr = (n: AstNode | undefined) => n && (n.type === 'MemberExpression' || n.type === 'StaticMemberExpression')
 ...
 */
function isLocalReference(name: string, functionParams: Set<string>, localVars: Set<string>): boolean {
 ...
 * 同时支持 ESTree 和 OXC AST 格式
 */
function collectIdentifiers(node: unknown, identifiers: Set<string>, zodIdentifier: string): void {
...
function hasComputedProperty(node: unknown): boolean {
  ...
  for (const key of Object.keys(n)) {
    ...
    if (Array.isArray(value)) {
      for (const item of value) {
        if (hasComputedProperty(item)) return true
      }
    } else if (value && typeof value === 'object') {
      if (hasComputedProperty(value)) return true
    }
  }
 ...
 */
export function analyze(code: string, filename: string, zodIdentifiers: string[], variablePrefix: string): AnalyzeResult {
  ...
  const isIdent = (n: Record<string, unknown>) => n.type === 'Identifier' || n.type === 'BindingIdentifier'
  ...
  for (const { node, depth, params, localVars } of candidates) {
    ...
    if (isContainedInCovered(node.start, node.end)) continue

    // Check if contains dynamically computed property keys / 检查是否包含动态计算的属性键
    if (hasComputedProperty(node)) continue
