...
export default function hmrNotifyPlugin(options?: HmrNotifyOptions): Plugin {
  ...
  function printChanges() {
    ...
    const filesText = pendingFiles.length === 1 ? colors.cyan(pendingFiles[0]) : `${colors.cyan(pendingFiles[0])} ${colors.dim(`等 ${pendingFiles.length} 个文件`)}`

    server.config.logger.info(`${colors.green('[hmr]')} ${filesText} ${colors.dim('已变更')}`, { timestamp: true })
