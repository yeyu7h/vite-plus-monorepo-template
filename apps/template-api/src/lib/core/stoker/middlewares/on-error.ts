...
const onError: ErrorHandler = (err, c) => {
  ...
  if (extracted) {
    ...
    logger.warn({ code: pgError.code, query, params }, '[Stoker]: pg error')
    return c.json(Resp.fail(message), statusCode)
  }

  // 2. Other errors (keep original logic) / 其他错误（保持原逻辑）
  const currentStatus = 'status' in err ? err.status : c.newResponse(null).status
  const statusCode = currentStatus !== HttpStatusCodes.OK ? (currentStatus as ContentfulStatusCode) : HttpStatusCodes.INTERNAL_SERVER_ERROR

  const env = c.env?.NODE_ENV || ProcessEnv?.NODE_ENV
  return c.json(
    Resp.fail(err.message, {
