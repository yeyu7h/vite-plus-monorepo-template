...
export const list: DictRouteHandlerType<'list'> = async (c) => {
  ...
  void redisClient.setex(DICT_LIST_CACHE_KEY, DICT_CACHE_TTL, JSON.stringify(result)).catch((error) => logger.warn({ error }, '[字典]: Redis 列表缓存写入失败'))
...
export const getByCode: DictRouteHandlerType<'getByCode'> = async (c) => {
  ...
  void redisClient.setex(cacheKey, DICT_CACHE_TTL, JSON.stringify(result)).catch((error) => logger.warn({ error, code }, '[字典]: Redis 缓存写入失败'))
