...
import { addDefaultSorting, convertFiltersToSQL, convertSortersToSQL, FiltersConverter, SortersConverter, validateFilterFields, validateSorterFields } from '../converters'
...
describe('refine-query Converters', () => {
  ...
  describe('validateFilterFields', () => {
    it('应该验证所有字段有效', () => {
      const result = validateFilterFields([{ field: 'name', operator: 'eq', value: 'test' }], testTable)
    ...
    it('应该检测无效字段', () => {
      const result = validateFilterFields([{ field: 'unknown', operator: 'eq', value: 'test' }], testTable)
    ...
    it('应该支持 allowedFields 白名单', () => {
      const result = validateFilterFields([{ field: 'name', operator: 'eq', value: 'test' }], testTable, ['name', 'status'])

      expect(result.valid).toBe(true)
    })

    it('应该拒绝不在白名单中的字段', () => {
      const result = validateFilterFields([{ field: 'age', operator: 'eq', value: 18 }], testTable, ['name', 'status'])
  ...
  describe('validateSorterFields', () => {
    it('应该验证所有字段有效', () => {
      const result = validateSorterFields([{ field: 'name', order: 'asc' }], testTable)
    ...
    it('应该检测无效字段', () => {
      const result = validateSorterFields([{ field: 'unknown', order: 'asc' }], testTable)
    ...
    it('应该支持 allowedFields 白名单', () => {
      const result = validateSorterFields([{ field: 'name', order: 'asc' }], testTable, ['name', 'createdAt'])

      expect(result.valid).toBe(true)
    })

    it('应该拒绝不在白名单中的字段', () => {
      const result = validateSorterFields([{ field: 'age', order: 'asc' }], testTable, ['name', 'createdAt'])
  ...
  describe('SQL 注入防护', () => {
    ...
    describe('字段名注入尝试', () => {
      ...
      it.each(fieldInjectionPayloads)('validateFilterFields 应该检测恶意字段名: %s', (maliciousField) => {
        const result = validateFilterFields([{ field: maliciousField, operator: 'eq', value: 'test' }], testTable)
      ...
      it.each(fieldInjectionPayloads)('validateSorterFields 应该检测恶意排序字段: %s', (maliciousField) => {
        const result = validateSorterFields([{ field: maliciousField, order: 'asc' }], testTable)
