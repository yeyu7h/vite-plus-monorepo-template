...
export class RequestClient {
  ...
  constructor(options: RequestClientOptions = {}) {
    const defaultConfig: RequestClientOptions = { headers: { 'Content-Type': 'application/json;charset=utf-8' }, responseReturn: 'raw', timeout: 10_000 }
  ...
  public patch<T = unknown>(url: string, data?: unknown, config?: RequestClientConfig): Promise<T> {
  ...
  public async request<T>(url: string, config: RequestClientConfig = {}): Promise<T> {
    try {
      ...
      const response: AxiosResponse<T> = await this.instance({ url, ...axiosConfig, ...(paramsSerializer ? { paramsSerializer: getParamsSerializer(paramsSerializer) } : {}) })
