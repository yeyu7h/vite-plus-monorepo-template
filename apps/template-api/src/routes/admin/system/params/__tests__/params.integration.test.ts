...
async function cleanupTestParams(): Promise<void> {
  try {
    ...
    await db.delete(systemParams).where(like(systemParams.key, 'test_%'))
...
describe('system params routes', () => {
  ...
  describe('authentication & authorization', () => {
    it('should require authentication', async () => {
      const response = await client.system.params.$get({ query: {} }, {})

      expect(response.status).toBe(HttpStatusCodes.UNAUTHORIZED)
    })

    it('should allow authenticated admin requests', async () => {
      const response = await client.system.params.$get({ query: {} }, { headers: getAuthHeaders(adminToken) })
  ...
  describe('get /system/params - list params', () => {
    ...
    it('should support filtering by key', async () => {
      ...
      const response = await client.system.params.$get(
        {
          query: {
            filters: JSON.stringify([{ field: 'key', operator: 'contains', value: 'test_filter' }]),
          },
        },
        { headers: getAuthHeaders(adminToken) },
      )
      ...
      if (response.status === HttpStatusCodes.OK) {
        ...
        expect(items.some((param: { key: string }) => param.key.includes('test_filter'))).toBe(true)
      }
    })
  })
  ...
  describe('get /system/params/{id} - get param', () => {
    ...
    it('should validate UUID format', async () => {
      const response = await client.system.params[':id'].$get({ param: { id: 'invalid-uuid' } }, { headers: getAuthHeaders(adminToken) })
    ...
    it('should return 404 for non-existent param', async () => {
      ...
      const response = await client.system.params[':id'].$get({ param: { id: nonExistentId } }, { headers: getAuthHeaders(adminToken) })

      expect(response.status).toBe(HttpStatusCodes.NOT_FOUND)
    })

    it('should get param details', async () => {
      const response = await client.system.params[':id'].$get({ param: { id: paramId } }, { headers: getAuthHeaders(adminToken) })
  ...
  describe('delete /system/params/{id} - delete param', () => {
    it('should validate UUID format', async () => {
      const response = await client.system.params[':id'].$delete({ param: { id: 'invalid-uuid' } }, { headers: getAuthHeaders(adminToken) })
    ...
    it('should return 404 for non-existent param', async () => {
      const nonExistentId = crypto.randomUUID()
      const response = await client.system.params[':id'].$delete({ param: { id: nonExistentId } }, { headers: getAuthHeaders(adminToken) })
    ...
    it('should delete param successfully', async () => {
      ...
      const deleteResponse = await client.system.params[':id'].$delete({ param: { id: deleteTestParamId } }, { headers: getAuthHeaders(adminToken) })
      ...
      const verifyResponse = await client.system.params[':id'].$get({ param: { id: deleteTestParamId } }, { headers: getAuthHeaders(adminToken) })
