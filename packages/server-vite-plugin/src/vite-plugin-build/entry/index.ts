...
export type EntryContentHook = (appName: string, options?: EntryContentHookOptions) => string | Promise<string>
...
export const getEntryContent = async (options: GetEntryContentOptions) => {
  const preset = presets.includes(options.preset ?? 'hono')
    ? (options.preset ?? 'hono')
    : (console.warn(`Invalid preset: ${options.preset}. Must be one of: ${presets.join(', ')}. Using 'hono' as default.`), 'hono')
...
${await hooksToString('mainApp', [defaultExportHook])}`
  ...
  const defaultExportHook = options.entryContentDefaultExportHook ?? (() => 'export default app')
