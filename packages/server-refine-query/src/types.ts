...
export type ConditionalFilter = {
  key?: string
  operator: 'or' | 'and'
  value: Array<z.infer<typeof LogicalFilterSchema> | ConditionalFilter>
}
 ...
 * 条件过滤器 Schema (递归定义)
 */
export const ConditionalFilterSchema: z.ZodType<ConditionalFilter> = z.lazy(() =>
  z.object({
 ...
 * CRUD 过滤器 Schema
 */
export const CrudFilterSchema = z.union([LogicalFilterSchema, ConditionalFilterSchema])
...
function getDepth(obj: unknown, depth = 0): number {
  if (depth > 10) return depth // Prevent infinite recursion / 防止无限递归
 ...
 * 用于验证来自前端的查询参数
 */
export const RefineQueryParamsSchema = z
  .object({
    ...
    filters: z
      .preprocess(safeJsonPreprocess, CrudFiltersSchema.optional())
      .optional()
      .openapi({
        type: 'string',
    ...
    sorters: z
      .preprocess(safeJsonPreprocess, CrudSortingSchema.optional())
      .optional()
      .openapi({
        type: 'string',
...
export type FilterFieldsValidator = (filters: CrudFilters, table: PgTable, allowedFields?: readonly string[]) => FieldValidationResult

/** Sorter fields validator function type / 排序字段验证器函数类型 */
export type SorterFieldsValidator = (sorters: CrudSorting, table: PgTable, allowedFields?: readonly string[]) => FieldValidationResult
