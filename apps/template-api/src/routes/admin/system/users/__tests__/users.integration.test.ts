...
function createSysUsersApp() {
  return createTestApp()
    .use('/system/users/*', jwt({ secret: env.ADMIN_JWT_SECRET, alg: 'HS256' }))
...
async function cleanupTestUsers(): Promise<void> {
  try {
    await db.transaction(async (tx) => {
      ...
      await tx.delete(systemUserRoles).where(or(...userIds.map((id) => eq(systemUserRoles.userId, id))))

      // Delete Casbin rules (if there are user-related rules) / 删除 Casbin 规则（如果有用户相关的规则）
      await tx.delete(casbinRule).where(and(eq(casbinRule.ptype, 'g'), or(...userIds.map((id) => eq(casbinRule.v0, id)))))

      // Delete users themselves / 删除用户本身
      await tx.delete(systemUsers).where(or(like(systemUsers.username, 't123_%'), like(systemUsers.username, 'test_%'), like(systemUsers.username, 't%_test')))
    })
  } catch (error) {
...
describe('system user routes', () => {
  ...
  describe('authentication & authorization', () => {
    it('should allow authenticated admin requests', async () => {
      const response = await client.system.users.$get({ query: {} }, { headers: getAuthHeaders(adminToken) })

      expect(response.status).toBe(HttpStatusCodes.OK)
    })

    it('should deny access with user token (no admin permissions)', async () => {
      const response = await client.system.users.$get({ query: {} }, { headers: getAuthHeaders(userToken) })

      expect(response.status).toBe(HttpStatusCodes.FORBIDDEN)
    })

    it('should require authentication', async () => {
      const response = await client.system.users.$get({ query: {} }, {})
  ...
  describe('get /system/user - list users', () => {
    ...
    it('should support filtering by username', async () => {
      const response = await client.system.users.$get(
        {
          query: {
            filters: JSON.stringify([{ field: 'username', operator: 'contains', value: 'admin' }]),
          },
        },
        { headers: getAuthHeaders(adminToken) },
      )
      ...
      if (response.status === HttpStatusCodes.OK) {
        ...
        expect(items.some((user: { username: string }) => user.username.includes('admin'))).toBe(true)
      }
    })
  ...
  describe('get /system/user/{id} - get user', () => {
    ...
    it('should validate UUID format', async () => {
      const response = await client.system.users[':id'].$get({ param: { id: 'invalid-uuid' } }, { headers: getAuthHeaders(adminToken) })
    ...
    it('should return 404 for non-existent user', async () => {
      ...
      const response = await client.system.users[':id'].$get({ param: { id: nonExistentId } }, { headers: getAuthHeaders(adminToken) })
    ...
    it('should get user details', async () => {
      const response = await client.system.users[':id'].$get({ param: { id: userId } }, { headers: getAuthHeaders(adminToken) })
  ...
  describe('patch /system/user/{id} - update user', () => {
    ...
    beforeAll(async () => {
      ...
      const adminResponse = await client.system.users.$get(
        {
          query: {
            filters: JSON.stringify([{ field: 'username', operator: 'eq', value: 'admin' }]),
          },
        },
        { headers: getAuthHeaders(adminToken) },
      )
  ...
  describe('delete /system/user/{id} - delete user', () => {
    ...
    it('should validate UUID format', async () => {
      const response = await client.system.users[':id'].$delete({ param: { id: 'invalid-uuid' } }, { headers: getAuthHeaders(adminToken) })
    ...
    it('should return 403 when deleting built-in user', async () => {
      ...
      const response = await client.system.users[':id'].$delete({ param: { id: builtInUserId } }, { headers: getAuthHeaders(adminToken) })
    ...
    it('should return 404 for non-existent user', async () => {
      const nonExistentId = crypto.randomUUID()
      const response = await client.system.users[':id'].$delete({ param: { id: nonExistentId } }, { headers: getAuthHeaders(adminToken) })
    ...
    it('should delete user successfully', async () => {
      ...
      const deleteResponse = await client.system.users[':id'].$delete({ param: { id: deleteTestUserId } }, { headers: getAuthHeaders(adminToken) })
