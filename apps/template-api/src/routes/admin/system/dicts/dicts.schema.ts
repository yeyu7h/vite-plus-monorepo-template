...
/** Dictionary item Schema / 字典项 Schema */
...
/** Dictionary code field / 字典编码字段 */
export const dictCodeField = z
  .string()
  .min(1, '字典编码不能为空')
  .max(64, '字典编码最多64个字符')
...
/** Dictionary base fields (shared) / 字典基础字段（共享） */
...
/** Query params Schema (custom filter fields, excluding pagination params) / 查询参数 Schema（自定义过滤字段，不包含分页参数） */
