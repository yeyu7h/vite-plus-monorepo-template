...
import type { ConfigEnv, Plugin, ResolvedConfig, UserConfig } from 'vite-plus'
...
export const defaultOptions: Required<
  Omit<BuildOptions, 'bundleDeps' | 'entryContentAfterHooks' | 'entryContentBeforeHooks' | 'entryContentDefaultExportHook' | 'nativeAssets' | 'nativeDeps' | 'targetPlatform'>
> = {
 ...
 */
function resolvePlatformSubPackages(rootDir: string, pkg: string, targetPlatform: TargetPlatform): Record<string, string> {
...
function installAndExtractNativeDeps(rootDir: string, outDir: string, nativeDeps: string[], targetPlatform?: TargetPlatform): void {
  ...
  try {
    unlinkSync(resolve(outDir, 'package.json'))
  } catch {}
  try {
    unlinkSync(resolve(outDir, 'package-lock.json'))
  } catch {}
}
...
const buildPlugin = (options: BuildOptions): Plugin => {
  ...
  return {
    ...
    renderChunk(code) {
      ...
      return code.replace(/(__require|require)\(["']\.\/([^"']*\.node)["']\)/g, (_, fn, file) => {
    ...
    config: async (): Promise<UserConfig> => {
      ...
      if (bundleDeps) {
        ...
        externalList = [...builtinModules, /^node:/, ...nativeBinaryPatterns, ...(options?.external ?? [])]
      } else {
