...
jobs:
  deploy:
    steps:
      - name: Run migrations
        run: vp run @app/template-api#migrate
        env:
...
| 命令 | 用途 | 场景 |
|------|------|------|
| `vp run @app/template-api#push` | 直接同步 schema 到数据库 | 开发环境快速迭代 |
| `vp run @app/template-api#generate` | 生成迁移 SQL 文件 | 准备生产部署 |
| `vp run @app/template-api#migrate` | 执行迁移文件 | 生产环境部署 |
