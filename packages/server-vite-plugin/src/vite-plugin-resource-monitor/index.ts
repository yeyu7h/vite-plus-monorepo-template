      ...
      server.watcher.on('change', async (file) => {
        ...
        if (!baselineSnapshot) {
          ...
          if (snapshotsEqual(currentSnapshot, lastStableSnapshot)) {
            stabilizeCounter++
            server.config.logger.info(colors.dim(`[资源监控] 等待稳定中... (${stabilizeCounter}/${stabilizeCount})`))
            ...
            if (stabilizeCounter >= stabilizeCount) {
              ...
              server.config.logger.info(`${colors.green('[资源监控]')} 基线已记录: ${colors.cyan(formatSnapshot(baselineSnapshot))}`)
            }
          } else {
            // Resource changed, reset counter / 资源变化，重置计数
            lastStableSnapshot = currentSnapshot
            stabilizeCounter = 1
            server.config.logger.info(colors.dim(`[资源监控] 资源变化，重新等待稳定... (${stabilizeCounter}/${stabilizeCount})`))
          }
          return
        }
        ...
        if (isGrowing && details.length > 0) {
          ...
          server.config.logger.warn(`  ${colors.dim('触发文件:')} ${colors.cyan(relativePath)}`)
          server.config.logger.warn(`  ${colors.dim('HMR 次数:')} ${hmrCount}`)
          server.config.logger.warn(`  ${colors.dim('连续增长:')} ${colors.red(`${consecutiveGrowths} 次`)}`)
          server.config.logger.warn(`  ${colors.dim('累计增长:')} ${colors.red(`+${totalGrowth} 个连接`)}`)

          server.config.logger.warn(colors.dim('  详情:'))
          for (const { type, baseline, current, diff } of details) {
            server.config.logger.warn(`    ${colors.dim(`${type}:`)} ${baseline} → ${current} ${colors.red(`(+${diff})`)}`)
          }

          server.config.logger.warn(colors.dim('\n  提示: 连接数持续增长可能存在泄漏，建议:'))
          server.config.logger.warn(colors.dim('    1. 使用 createSingleton 包裹数据库/Redis 客户端'))
          server.config.logger.warn(colors.dim('    2. 检查是否有未清理的连接或监听器'))
          server.config.logger.warn(colors.dim('  参考: src/lib/internal/singleton.ts\n'))
        }
      })
    },
  }
