...
export default function zodHoistPlugin(options?: ZodHoistOptions): Plugin {
  ...
  return {
    ...
    transform(code, id) {
      ...
      // Analyze code / 分析代码
      const analyzeResult = analyze(code, id, opts.zodIdentifiers, opts.variablePrefix)
      ...
      if (opts.debug) {
        console.warn(`[zod-hoist] ${id}: 提升 ${analyzeResult.hoistableSchemas.length} 个 schema`)
