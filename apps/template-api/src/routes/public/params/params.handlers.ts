...
export const list: ParamRouteHandlerType<'list'> = async (c) => {
  ...
  void redisClient.setex(PARAM_LIST_CACHE_KEY, PARAM_CACHE_TTL, JSON.stringify(result)).catch((error) => logger.warn({ error }, '[参数]: Redis 列表缓存写入失败'))
...
export const getByKey: ParamRouteHandlerType<'getByKey'> = async (c) => {
  ...
  void redisClient.setex(cacheKey, PARAM_CACHE_TTL, JSON.stringify(result)).catch((error) => logger.warn({ error, key }, '[参数]: Redis 缓存写入失败'))
