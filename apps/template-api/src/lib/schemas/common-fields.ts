...
export const usernameField = z.string().min(4, '用户名最少4个字符').max(32, '用户名最多32个字符').regex(/^\w+$/, '用户名只能包含字母、数字和下划线').meta({ description: '用户名' })

/** Password field / 密码字段 */
export const passwordField = z.string().min(6, '密码最少6个字符').max(20, '密码最多20个字符').meta({ description: '密码' })

/** Nickname field / 昵称字段 */
export const nicknameField = z.string().min(1, '昵称不能为空').max(32, '昵称最多32个字符').meta({ description: '用户昵称' })

/** Email field / 邮箱字段 */
export const emailField = z.email('邮箱格式不正确').meta({ description: '邮箱地址' })
