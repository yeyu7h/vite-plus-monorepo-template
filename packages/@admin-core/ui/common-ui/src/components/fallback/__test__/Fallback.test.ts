...
describe('default states', () => {
  test.each([
    ...
    ['coming-soon', '敬请期待', '此功能正在建设中，敬请期待。', undefined],
    ...
    expect(wrapper.find('[data-fallback-icon]').exists()).toBe(status === 'coming-soon' || status === 'offline')
...
test.each(['403', '404', '500'] as const)('renders %s as a number instead of an icon', (status) => {
  const wrapper = mountFallback({ props: { status } })
...
test('supports custom copy, image, and action visibility', () => {
  ...
  expect(wrapper.get('img').attributes()).toMatchObject({ src: '/custom-fallback.png' })
  expect(wrapper.get('img').attributes('aria-hidden')).toBe('true')
  expect(wrapper.get('img').element.parentElement?.getAttribute('aria-hidden')).toBe('true')
