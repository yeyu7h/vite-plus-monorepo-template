...
const REFRESH_TTL_SECONDS = differenceInSeconds(calculateExpiration(REFRESH_TOKEN_DURATION), new Date())
...
export async function refreshAccessToken(refreshToken: string) {
  ...
  const [newAccessToken, newRefreshToken] = await Promise.all([generateAccessToken(user), generateRefreshToken(user)])
