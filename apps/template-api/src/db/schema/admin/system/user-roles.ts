...
export const systemUserRoles = snakeCase.table(
  'system_user_roles',
  {
    userId: uuid()
      .notNull()
  ...
  (table) => [primaryKey({ columns: [table.userId, table.roleId] }), index('idx_user_roles_user_id').on(table.userId), index('idx_user_roles_role_id').on(table.roleId)],
)
