...
function getIpVersion(ip: string): 0 | 4 | 6 {
  if (Ipv4Schema.safeParse(ip).success) return 4
  if (Ipv6Schema.safeParse(ip).success) return 6
  return 0
}
...
const fetchLoc = async (ip: string) => {
  ...
  if (e3 || !p || typeof p !== 'object') return UNKNOWN
  const v = S.safeParse(p)
  if (!v.success || v.data.err) return UNKNOWN
...
const getOrFetch = async (ip: string) => {
  ...
  if (!e && c) return c === NULL_CACHE_VALUE ? UNKNOWN : c
...
export async function getIPAddress(ip: string): Promise<string> {
  ...
  if (n === UNKNOWN) return LOCAL // Keep current behavior: unknown is also treated as "local" / 保持你当前行为：unknown 也按"本地"处理
  const v = getIpVersion(n)
  if (!n || v === 0) return UNKNOWN
  if (isPrivateIp(n, v)) return LOCAL
