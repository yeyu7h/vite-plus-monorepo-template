  ...
  describe('getIPAddress', () => {
    ...
    describe('公网IP处理（带Redis缓存）', () => {
      ...
      it('缓存未命中时应调用API并缓存结果', async () => {
        ...
        const mockResponse = {
          ok: true,
          arrayBuffer: vi.fn().mockResolvedValue(Buffer.from(JSON.stringify({ addr: '广东省深圳市' }))),
        }
      ...
      it('API返回pro和city时应拼接', async () => {
        ...
        const mockResponse = {
          ok: true,
          arrayBuffer: vi.fn().mockResolvedValue(Buffer.from(JSON.stringify({ pro: '广东省', city: '深圳市' }))),
        }
        mockFetch.mockResolvedValue(mockResponse)
      ...
      it('API返回err字段应返回unknown', async () => {
        ...
        const mockResponse = {
          ok: true,
          arrayBuffer: vi.fn().mockResolvedValue(Buffer.from(JSON.stringify({ err: 'IP not found' }))),
        }
        mockFetch.mockResolvedValue(mockResponse)
      ...
      it('Redis读取失败时应继续调用API', async () => {
        ...
        const mockResponse = {
          ok: true,
          arrayBuffer: vi.fn().mockResolvedValue(Buffer.from(JSON.stringify({ addr: '上海市' }))),
        }
        mockFetch.mockResolvedValue(mockResponse)
