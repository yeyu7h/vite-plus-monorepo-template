 ...
 */
export const systemParams = snakeCase.table(
  'system_params',
  {
    ...baseColumns,
...
export const selectSystemParamsSchema = createSelectSchema(systemParams, {
  ...
  valueType: z
    .enum([ParamValueType.STRING, ParamValueType.NUMBER, ParamValueType.BOOLEAN, ParamValueType.JSON])
    .meta({ description: '参数值类型 (STRING=字符串, NUMBER=数字, BOOLEAN=布尔值, JSON=JSON对象)' }),
